...
fits2 <- runWorkflow(ent$cacheDir, workflow="snm", bio.var=X, adj.var=model.matrix(~ u$v[,1]))


# Get the data
dat <- exprs(fits$hgu133plus2[[1]])
# Perform a differential expression test.
treatment <- ifelse(grepl('Src', list.files(ent$cacheDir)), "GFP", "Src")
...
sva.fit <- sva(dat, bio.var=X, n.sv=2, num.iter=30, diagnose=FALSE)
